...
const VIDEO_SERVER_RESPONSE = {
  body: {
    ...
    'results': [{
      ...
      'advertiserDomains': ['apesterbid.com'],
      'exp': 60,
      'width': 545,
      'height': 307,
      'mediaType': 'video',
  ...
  describe('getUserSyncs', function () {
    it('should have valid user sync with iframeEnabled', function () {
      ...
      expect(result).to.deep.equal([{
        type: 'iframe',
        url: 'https://sync.apesterbid.com/api/sync/iframe/?cid=testcid123&gdpr=0&gdpr_consent=&us_privacy='
      }]);
    });
    ...
    it('should have valid user sync with pixelEnabled', function () {
      ...
      expect(result).to.deep.equal([{
        'url': 'https://sync.apesterbid.com/api/sync/image/?cid=testcid123&gdpr=0&gdpr_consent=&us_privacy=',
        'type': 'image'
      }]);
    });
    ...
    it('should generate url with consent data', function () {
      ...
      expect(result).to.deep.equal([{
        'url': 'https://sync.apesterbid.com/api/sync/image/?cid=testcid123&gdpr=1&gdpr_consent=consent_string&us_privacy=usp_string&gpp=gpp_string&gpp_sid=7',
        'type': 'image'
      }]);
    });
  });
